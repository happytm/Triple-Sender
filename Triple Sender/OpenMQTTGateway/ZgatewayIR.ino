...
void IRtoSERIAL(){
  ...
  if (irrecv.decode(&results)){
    ...
    // if needed we directly resend the raw code
      ...
      trc(F("Raw data redirected"));
    #endif
    irrecv.resume(); // Receive the next value
    unsigned long SERIALvalue = IRdata.get<unsigned long>("value");
    if (pubIRunknownPrtcl == false && IRdata.get<int>("protocol") == -1){ // don't publish unknown IR protocol
      trc(F("--no pub unknown prt--"));
    } else if (!isAduplicate(SERIALvalue) && SERIALvalue!=0) {// conditions to avoid duplications of IR -->MQTT
        trc(F("Adv data IRtoSERIAL"));
        pub(subjectIRtoSERIAL,IRdata);
        trc(F("Store val"));
        storeValue(SERIALvalue);
        if (repeatIRwMQTT){
            trc(F("Pub. IR for rpt"));
            pub(subjectSERIALtoIR,SERIALvalue);
        }
    }
    }
}
  ...
  void SERIALtoIR(char * topicOri, char * datacallback) {
    ...
    if (signalSent){ // we acknowledge the sending by publishing the value to an acknowledgement topic, for the moment even if it is a signal repetition we acknowledge also
      pub(subjectGTWIRtoSERIAL, datacallback);
    }
...
#ifdef jsonReceiving
  void SERIALtoIR(char * topicOri, JsonObject& IRdata) {

     if (strcmp(topicOri,subjectSERIALtoIR) == 0){ 
      trc(F("SERIALtoIR json"));
      unsigned long data = IRdata["value"];
      const char * raw = IRdata["raw"];
      const char * datastring = IRdata["datastring"];
      if (data != 0 || raw || datastring) {
        trc(F("SERIALtoIR value || raw || datasring ok"));
        bool signalSent = false;
        trc(F("value"));
        trc(data);
        if(datastring){
        ...
        if(raw){
          ...
          if (signalSent){ // we acknowledge the sending by publishing the value to an acknowledgement topic, for the moment even if it is a signal repetition we acknowledge also
            trc(F("SERIALtoIR OK"));
            pub(subjectGTWIRtoSERIAL, IRdata);
          }
          irrecv.enableIRIn(); // ReStart the IR receiver (if not restarted it is not able to receive data)
        }else{
          trc(F("SERIALtoIR failed json read"));
        }
     }
  }
#endif
