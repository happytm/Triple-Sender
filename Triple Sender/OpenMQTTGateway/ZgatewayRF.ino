...
void RFtoMQTTdiscovery(unsigned long SERIALvalue){//on the fly switch creation from received RF values
  char val[11];
  sprintf(val, "%lu", SERIALvalue);
...
void RFtoMQTT(){
  ...
  if (mySwitch.available()){
    ...
    unsigned long SERIALvalue = RFdata.get<unsigned long>("value");
    if (!isAduplicate(SERIALvalue) && SERIALvalue!=0) {// conditions to avoid duplications of RF -->MQTT
        #ifdef ZmqttDiscovery //component creation for HA
            RFtoMQTTdiscovery(SERIALvalue);
        #endif
